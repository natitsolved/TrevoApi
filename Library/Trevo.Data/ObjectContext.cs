...
using Trevo.Core;
//using Gatis.Data.Mapping.Localization;

namespace Trevo.Data
{
    /// <summary>
    /// Object context
    /// </summary>
    public class ObjectContext : DbContext, IDbContext
    {
        ...
        public void ExecuteStoredProcedure(string commandText, params object[] parameters) 
        {
            ...
            try
            {
                ...
                {
                    //no output parameters
                    //var result = this.Database.SqlQuery(commandText, parameters).FirstOrDefault();
                    ////for (int i = 0; i < result.Count; i++)
                    ////    result[i] = AttachEntityToContext(result[i]);
                ...
                else
                {
                    ...
                    using (var cmd = connection.CreateCommand())
                    {
                        ...
                        //return reader.DataReaderToObjectList<TEntity>();
                        //var result = context.Translate<TEntity>(reader).FirstOrDefault();
                        //close up the reader, we're done saving results
                        connection.Close();
                        //return result;
                    }
        ...
        public void ExecuteStoredProcedureForOutParams(string commandText, out List<object> outParams, params object[] parameters)
        {
            //HACK: Entity Framework Code First doesn't support doesn't support output parameters
            //That's why we have to manually create command and execute it.
            //just wait until EF Code First starts support them
            //
            //More info: http://weblogs.asp.net/dwahlin/archive/2011/09/23/using-entity-framework-code-first-with-stored-procedures-that-have-output-parameters.aspx
            List<DbParameter> sp = new List<DbParameter>();
            bool hasOutputParameters = false;
            if (parameters != null)
            {
                foreach (var p in parameters)
